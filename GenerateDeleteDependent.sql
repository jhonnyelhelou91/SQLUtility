IF EXISTS (SELECT 1 FROM sys.objects where [object_id] = OBJECT_ID('Utility.GenerateDeleteDependent'))
	DROP FUNCTION [Utility].GenerateDeleteDependent
GO

CREATE FUNCTION Utility.GenerateDeleteDependent
(
	@table nvarchar(100),
	@Where nvarchar(max)
)
